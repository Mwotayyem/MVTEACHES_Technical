...
| **الأساس الجديد** | «MVTEACHES JORDAN — OFFICIAL BUSINESS DECISIONS» بتاريخ 2026-08-17 — **مصدر مُلزِم يعلو على كل افتراضات v1** |
...
> **بقي `ACTION REQUIRED` من v1 دون حل:** صفحات SRS رقم 1 و3 (نظرة عامة، نطاق العمل، البنية العامة للفروع) **ما تزال مفقودة**. S0 عالج معظم أثرها، لكن اطلب الوثيقة كاملة قبل تجميد النطاق.
...
| **D-16** | **مدة الاشتراك 45 يومًا** | صلاحية ثابتة | `Subscription.ExpiresOn` = البداية + 45 يومًا (قابل للضبط) |
...
| **D-19** | **صلاحية التعويض 30 يومًا** | حيث ينطبق مفهوم التعويض فعليًا | `MakeUpCredit.ExpiresOn` |
...
| **D-29** | **امتحان تحديد المستوى قائم** | Conversation + Grammar + Writing · **عبر Zoom** · المعلمون يضعون الأسئلة | نموذج هجين: أسئلة + محاولة + **تقييم بشري** |
...
| هل نموذج العمل متسق داخليًا الآن؟ | **شبه متسق.** حُسم 90%، وبقيت **4 تناقضات حقيقية** يجب حسمها قبل الجداول (القسم 42). |
| ما أخطر ما فيه الآن؟ | **قناة الدفع.** D-11 قد يلغي قبول البطاقات كليًا. هذا سؤال تجاري لا تقني، وجوابه يغيّر الشاشات والتكلفة والتشغيل. |
...
| هل التكلفة التشغيلية معقولة؟ | **نعم، وأقل من V1.** بلا عمولة بوابة: **≈ $79–108/شهر لـ MVP + فاتورة واتساب** (البنية التحتية منها $37 فقط). |
...
امتحان تحديد المستوى عبر Zoom (D-29)
    ↓ تقييم المعلم
...
| الحصص الخاصة (Private) | إيراد | ⚠️ **هل هي ضمن MVP؟ سؤال مفتوح `Q-09`** |
| امتحان تحديد المستوى | ⚠️ **UNKNOWN — مجاني أم مدفوع؟** | `Q-11` |
| الشهادات | ⚠️ **UNKNOWN — مجانية أم برسم؟** | `Q-12` |
...
| نتيجة امتحان المستوى | ✏️ | 👁️ | ✏️ تقييم | 👁️ أبنائه | 👁️ نفسه |
...
**ما يجب أن يكون مستقلًا تمامًا:** المستوى · نتيجة الامتحان · الاشتراك · الحضور · تاريخ الحصص · الواجبات · التقدّم · الشهادات · دفتر الاستحقاق.
...
[4] امتحان       → عبر Zoom: Conversation + Grammar + Writing (D-29)
...
| `PendingLevel` | تحقق، بلا مستوى | ✅ يتقدم للامتحان · ⚠️ **يُمنع من الاشتراك بمستوى محدد** حتى الإسناد |
| `Active` | مستوى معتمد + اشتراك ساري | كل شيء |
| `PaymentBlocked` | ⭐ عليه رصيد مستحق (D-14) | 👁️ يرى بياناته · ❌ **يُمنع من الحضور** · 📎 يرفع إثبات دفع |
| `Expired` | انتهى الاشتراك (45 يومًا) | يشتري اشتراكًا جديدًا · ⚠️ استحقاقه المتبقي — `CONF-04` |
| `Suspended` | إيقاف إداري | لا شيء — بسبب مُدقَّق |
| `Migrated` | ⭐ مُرحَّل، لم يفعّل حسابه (D-32) | استحقاقه محفوظ، ينتظر التفعيل |

> **قاعدة v1 المُبقى عليها (وتؤكدها S0 §5):** الطالب غير المحدد المستوى **لا يُمنع من الامتحان ولا من الوجود في النظام**، لكنه **يُمنع من الاشتراك في مستوى بعينه**، مع رسالة صريحة تدعوه لمراجعة الإدارة. هذا سلوك مقصود لا خطأ.

## 8.2 ⚠️ حالة حدّية لم تُحسم: الغياب بمبادرة الطالب

**S0 §12 و§13 يغطيان الإلغاء الذي يبدأه الأدمن فقط.** أما الطالب الذي **لا يحضر دون عذر** (No-Show)، أو **يعتذر قبل الحصة**، فلا توجد سياسة.

هذه ليست تفصيلة: إنها تحدد **سلوك** دفتر الاستحقاقات — هل يُنشأ قيد `Consumption` لطالب لم يحضر؟ انظر `Q-02` — 🔴 **محجوب لسلوك Phase 5، غير محجوب لشكل الجداول** (`reason` عمود `text` بقائمة موثّقة).
...
[7] تسليم الحصص → إقرار التسليم + تسجيل الحضور
...
# 10. Placement Test — امتحان تحديد المستوى

## 10.1 الواقع المعتمد (D-29)

**الامتحان قائم فعلًا ويُدار عبر Zoom، بثلاثة أقسام: Conversation, Grammar, Writing. المعلمون يضعون الأسئلة ويقيّمون.**

هذا يغيّر تصميم v1 جذريًا: v1 افترضت **امتحان اختيار من متعدد مُصحَّح آليًا**. الواقع **هجين**:

| القسم | طبيعته | التصحيح |
|---|---|---|
| **Grammar** | قابل للتصحيح الآلي (اختيار من متعدد) | ✅ آلي ممكن |
| **Writing** | نص حر | ❌ **بشري إلزامًا** |
| **Conversation** | شفهي عبر Zoom | ❌ **بشري إلزامًا** |

> **الاستنتاج المعماري:** النظام **ليس محرك امتحانات**. هو **أداة تسجيل تقييم بشري** مع دعم اختياري لبنك أسئلة الـ Grammar. **هذا يقلل الجهد كثيرًا** — بناء محرك امتحانات كامل كان سيكلف ~12 يومًا، والمطلوب فعليًا ~5.

## 10.2 النموذج المقترح

| الكيان | الغرض |
|---|---|
| `placement_tests` | تعريف الامتحان (نسخة، حالة، من أنشأه) |
| `placement_sections` | Conversation · Grammar · Writing — بأوزان |
| `placement_questions` | بنك أسئلة (نص، نوع، خيارات `jsonb`، إجابة، مستوى، من أنشأها) |
| `placement_attempts` | محاولة طالب: الموعد، رابط Zoom، المعلم المقيّم، الحالة |
| `placement_answers` | إجابات الطالب (للأقسام المسجّلة) |
| `placement_results` | ⭐ درجة كل قسم + الدرجة الكلية + **المستوى المقترح** + ملاحظات المعلم |
| `student_levels` | ⭐ **سجل تاريخي** للمستويات المسندة — لا حقل واحد |
...
## 10.4 `UNKNOWN` — حدود درجات المستويات

```
UNKNOWN: ما هي حدود الدرجات التي تُحوّل نتيجة الامتحان إلى A1…C2؟

WHY IT MATTERS: بدونها لا يمكن بناء الاقتراح الآلي للمستوى، ولا التحقق من
اتساق التقييم بين المعلمين. طالبان بنفس الأداء قد يُسندان لمستويين مختلفين.

OPTIONS:
  أ. أوزان لكل قسم + عتبات رقمية (مثلًا 0–20 → A1)
  ب. تقييم المعلم وحده هو الحاكم، والدرجة استرشادية فقط
  ج. هجين: النظام يقترح، والمعلم يعتمد أو يتجاوز بسبب

RECOMMENDATION: (ج). يعكس الواقع (Conversation و Writing بشريان)، ويوفّر
اتساقًا قابلًا للقياس، ويسمح بضبط العتبات لاحقًا كبيانات لا كود.
مطلوب من العميل: مصفوفة العتبات فقط — النظام يستوعب أي قيم.

CLIENT DECISION REQUIRED: YES — قبل بناء وحدة Assessment (Phase 3)، وليس قبل قاعدة البيانات.
```

## 10.5 `UNKNOWN` — إعادة التقييم

هل يجوز للطالب إعادة امتحان تحديد المستوى؟ كم مرة؟ بفاصل زمني؟ بمقابل مادي؟ — النموذج يدعم محاولات متعددة (`placement_attempts`)، **لكن السياسة غير محددة**. انظر `Q-11`.
...
> **لماذا `capacity` عمود وليس ثابت 4؟** الحصة الخاصة سعتها 1، وامتحان تحديد المستوى سعته 1، والحد 4 قد يتغيّر بقرار إداري. **الثوابت في الكود تُصبح ديونًا خلال شهور.** القيد `BETWEEN 1 AND 10` يحرس من الخطأ دون تجميد السياسة.
...
|---|---|---|---|
| **الحضور (Attendance)** | المعلم، **لكل طالب** | «هل حضر الطالب؟» | ⚠️ استحقاق **الطالب** |
| **التسليم (Delivery)** | المعلم **للحصة ككل**، ثم يتحقق الأدمن | «هل أُعطيت الحصة فعلًا؟» | ⭐ أجر **المعلم** |

**الحالات الأربع الممكنة — ولماذا لا يكفي حقل واحد:**

| # | الحصة سُلّمت؟ | الطالب حضر؟ | أجر المعلم | استحقاق الطالب |
|---|---|---|---|---|
| 1 | ✅ | ✅ | ✅ يُدفع | ✅ يُستهلك |
| 2 | ✅ | ❌ (تخلّف) | ✅ **يُدفع** | ⚠️ **`Q-02` — محجوب** |
| 3 | ❌ (المعلم غاب) | — | ❌ **لا يُدفع** | ❌ **لا يُستهلك** |
| 4 | ❌ (أُلغيت إداريًا) | — | ⚠️ `Q-07` | ❌ لا يُستهلك — يُنقل (D-20) |

> **الصف 2 هو الحالة التي تكشف صحة التصميم.** المعلم حضر وانتظر وأعطى الحصة لثلاثة طلاب من أربعة. **يستحق أجره كاملًا** — لأن أجره على الساعة المُسلَّمة لا على عدد الحضور (D-26). أما استحقاق الطالب المتخلف فسؤال منفصل تمامًا لم يُجب عنه.
>
> **لو كان لدينا حقل واحد `IsCompleted`، لاستحال التمييز بين الصفين 2 و3 — وهو الفرق بين دفع أجر واستقطاعه.**
...
| الحالة | المعنى | أثرها |
|---|---|---|
| `Present` | حضر | يستهلك الاستحقاق |
| `Absent` | لم يحضر بلا عذر | ⚠️ `Q-02` |
| `Excused` | عذر مقبول من الأدمن | ⚠️ `Q-02` — تعويض؟ |
| `Late` | تأخر | يستهلك (ما لم تُقرَّر سياسة أخرى) |
| `NotMarked` | المعلم لم يسجّل | 🚨 **يجب أن يُنبَّه عليها** |

> ⚠️ **`NotMarked` هو أخطر ما في هذا الجدول.** حصة مضت بلا تسجيل حضور تعني: استحقاق طالب معلّق، وأجر معلم معلّق، وتقدّم مستوى متوقف. **مهمة Hangfire يومية تُنبّه الأدمن بكل حصة مضى عليها 24 ساعة وحضورها غير مسجّل** — هذا ليس تحسينًا، هو حارس سلامة البيانات المالية.
...
```
   ...
   └────────────┘   └──────────────┘  └──────┬───────┘
          ...
          │                │          └──────┬───────┘
          ▼                ▼                 ▼
   لا أجر · لا استهلاك   لا أجر      ✅ أجر · ✅ استهلاك · ✅ تقدّم مستوى
   يُنقل الاستحقاق (D-20)              ✅ يدخل كشف الأجور
```
...
## 18.4 `UNKNOWN` — الدقيقة أم الساعة الكاملة؟

```
UNKNOWN: هل يُحتسب أجر المعلم بالساعة الكاملة أم بالدقائق الفعلية؟

WHY IT MATTERS: حصة انتهت بعد 50 دقيقة بدل 60 — هل يُدفع 12 JOD أم 10؟
على 100 حصة شهريًا، الفرق آلاف الدنانير سنويًا. وهو محل نزاع مع المعلمين
إن لم يُحسم كتابةً قبل الإطلاق.

OPTIONS:
  أ. الساعة الكاملة عن كل حصة مُسلَّمة (الأبسط والأشيع)
  ب. الدقائق الفعلية المتحقَّق منها
  ج. حد أدنى (مثلًا 45 دقيقة تُحتسب ساعة كاملة، وما دونها بالتناسب)

RECOMMENDATION: (أ) للحصص المجدولة القياسية — يطابق العقد الشائع ويقلل النزاع.
النموذج يخزّن الدقائق دائمًا، فيمكن تطبيق أي من الثلاثة كسياسة بيانات لا كود.

CLIENT DECISION REQUIRED: YES — 🟠 قبل Phase 6، غير محجوب لقاعدة البيانات.
```
...
CREATE TABLE subscriptions (
    ...
    starts_on         date NOT NULL,
    expires_on        date NOT NULL,           -- ⭐ = starts_on + 45 (D-16)
...
## 19.3 مدة الصلاحية والتمديد (D-16 / D-17 / D-18)

| الحالة | السلوك | التنفيذ |
|---|---|---|
| **إكمال طبيعي** لكل الحصص | ❌ **لا تمديد** (D-17) | `status = Completed` — ولا شيء آخر |
| **انتهاء 45 يومًا مع حصص متبقية** | ⚠️ **لا شيء آلي** | `status = Expired` + **تنبيه للأدمن** |
...
CREATE TABLE entitlement_ledger (
    ...
    expires_on      date NULL,                          -- ⭐ للتعويض: 30 يومًا (D-19)
    created_at_utc  timestamptz NOT NULL DEFAULT now()
);
...
| ماذا استُهلك؟ | `WHERE reason='Consumption'` |
...
5. ⭐ **قيد الاستهلاك يُكتب في نفس معاملة تحقق التسليم** — لا مهمة خلفية، لا تأخير.
...
## 22.2 🔴 التناقض الحرج `CONF-01` — اقرأ هذا قبل بناء أي شيء مالي
...
CREATE TABLE pricing_plans (
    ...
    validity_days   int NOT NULL DEFAULT 45,                 -- ⭐ D-16
    is_active       boolean NOT NULL DEFAULT true,
    effective_from  date NOT NULL,
...
> ⚠️ **`UNKNOWN`: هل الحصص الخاصة ضمن MVP؟** S0 لم يذكرها إطلاقًا، وS0 §43 لم يدرجها في قائمة MVP. **لا تفترض.** انظر `Q-09`.

---

# 24. Country & Currency — البلد والعملة

## 24.1 القرار (D-06 → D-09)

| البلد | العملة | التسعير |
|---|---|---|
| الأردن | **JOD** | قائمة الأردن |
| فلسطين | **ILS** | قائمة فلسطين |
...
CREATE TABLE countries (
    id                  int PRIMARY KEY,
    code                char(2) NOT NULL UNIQUE,      -- JO | PS
    name_ar             text NOT NULL,
    name_en             text NOT NULL,
    currency_code       char(3) NOT NULL,             -- JOD | ILS
...
| 5 | ⭐ **صلاحية اشتراكه المُرحَّل تُحدَّد يدويًا**، لا تُحسب بـ 45 يومًا من تاريخ الاستيراد |
...
> **30 ساعة تدريس لكل مستوى → شهادة ذلك المستوى.**

## 27.2 🔴 التناقض `CONF-03` — أين تُحسب الساعات؟

> **D-30:** المستوى يتطلب 30 ساعة.
> **D-16:** الاشتراك ينتهي بعد **45 يومًا**.
>
> **الحساب:** بحصتين أسبوعيًا (D-23)، 45 يومًا ≈ **12–13 حصة فقط**. أي أن المستوى الواحد يمتد على **2–3 اشتراكات متتالية**.

> ### ⛔ الخطأ القاتل الذي يجب تجنبه
>
> **لو رُبط تقدّم الشهادة بالاشتراك، فلن يحصل أي طالب على شهادة أبدًا** — لأن كل اشتراك يُغلق عند 12 حصة، والعتبة 30 ساعة.
>
> ### ✅ القاعدة الصحيحة
>
...
CREATE TABLE level_progress (           -- ⭐ عرض مادي يُحدَّث عند كل تحقق تسليم
    ...
    minutes_required  int NOT NULL,      -- من levels.required_minutes
...
|---|---|---|
| **امتحان تحديد المستوى** | D-29 | 1:1 — معلم وطالب |
...
| الإطار | ينطبق؟ | ملاحظة |
|---|---|---|
| **قانون حماية البيانات الأردني** | ✅ | التشريع الأساسي — ⚖️ راجعه مع محامٍ أردني |
...
> ⚠️ **مسألة لم تُطرح ويجب أن تُطرح:** أدمن مقيم في بريطانيا **يصل إلى بيانات أطفال أردنيين وفلسطينيين**. هذا **نقل بيانات عبر الحدود**. ⚖️ `LEGAL` — انظر `Q-18`.
...
| **مدة الاحتفاظ** | ⚠️ `UNKNOWN` — `Q-19` |
| **التشفير** | نقلًا وسكونًا |
| **تسجيل الوصول** | ⭐ **من فتح ملف قاصر ومتى** — سجل وصول لا سجل تعديل فقط |
| **إشعار الاختراق** | إجراء موثّق مسبقًا · ⚖️ المهلة القانونية `LEGAL` |
...
**النهج المقترح (⚖️ يتطلب إقرار محامٍ):**
...
| # | المخاطرة | التخفيف |
|---|---|---|
| 1 | ⚠️⚠️ **حصة خاصة 1:1 بين معلم وطفل** | ⭐ **موافقة صريحة من ولي الأمر · سياسة سلوك للمعلمين · فحص خلفية · ⚖️ مراجعة قانونية إلزامية** |
...
| 5 | **الأدمن المُطّلِع في بريطانيا** | نقل دولي — `Q-18` · ⚖️ `LEGAL` |
...
| الجدول | ملاحظة |
|---|---|
| `placement_tests` · `placement_sections` · `placement_questions` | بنك الأسئلة |
| `placement_attempts` · `placement_answers` · `placement_results` | ⭐ + تقييم بشري (D-29) |
...
| الجدول | ملاحظة |
|---|---|
| `pricing_plans` | ⭐ لكل (بلد × دورة × مستوى) (D-10) |
| `subscriptions` | ⭐ + `origin` + Snapshot السعر + 45 يومًا (D-13/D-16) |
...
| `payroll_periods` · `payroll_lines` | ⭐ **جديد** (D-26) |
...
| `entitlement_ledger` | ⭐ `UNIQUE (session_id, student_id) WHERE reason='Consumption'` | **لا خصم مزدوج** |
...
| تحقق التسليم | معاملة واحدة: `session_delivery` + قيد الاستهلاك + `level_progress` |
...
Modules/
  ...
  ├── Assessment/          ← امتحان المستوى + إسناد المستوى
...
## 38.1 تكلفة التطوير (لمرة واحدة)

**التقدير بأيام العمل — ❌ لن أخترع سعر يوم**، فهو يختلف جذريًا بين مطوّر منفرد وشركة، وبين الأردن وبريطانيا.

| الوحدة | أيام | ملاحظة |
|---|---|---|
| الأساس (مشروع، CI، Docker، Identity، MFA، RBAC) | 14 | |
| ⭐ **الأشخاص + أولياء الأمور + الوصاية والتخويل** | **12** | ⭐ جديد V2 (D-01) |
| الكتالوج (بلدان، عملات، دورات، مستويات، فئات، أسعار) | 8 | |
| ⭐ **امتحان المستوى (تسجيل تقييم بشري)** | **6** | ⬇️ أخف من v1 (D-29) |
| ⭐ **الجدولة + الجداول المتكررة + المولّد + المناطق الزمنية** | **18** | صافي أخف (D-23) |
| التسجيل في الحصص + الحضور | 7 | ⬇️ بلا محرك حجز حر |
| ⭐ **تحقق تسليم الحصة** | **6** | ⭐ جديد (D-26) |
| ⭐ **أجور المعلمين (أجور متغيرة + فترات + اعتماد)** | **14** | ⭐ جديد (D-27) |
| ⭐ **الاشتراكات + دفتر الاستحقاقات** | **14** | ⚠️ الأخطر |
| ⭐ **المدفوعات + المسار اليدوي + الأرصدة المستحقة** | **12** | معدّل جذريًا (D-11) |
| ⭐ **تقدّم المستوى + الشهادات** | **7** | ⭐ جديد (D-30) |
| Zoom (S2S OAuth + دورة حياة الاجتماع) | 6 | |
| واتساب + الإشعارات + Outbox + القوالب | 12 | |
| الملفات + الواجبات ثنائية الاتجاه + الأمن | 8 | |
| ⭐ **الترحيل والاستيراد** | **9** | ⭐ جديد (D-32) |
| لوحة الإدارة + التقارير + الفلاتر | 12 | |
| التوطين AR/EN + RTL | 6 | |
| الأمن (تشديد، حد معدل، تدقيق، اختبارات IDOR) | 10 | |
| الاختبارات (16 حالة إلزامية + تكامل) | 10 | |
| النشر + المراقبة + النسخ + الوثائق | 8 | |
| **الإجمالي — MVP** | **≈ 199 يومًا** | |
| احتياطي مخاطر 15% | +30 | |
| **الإجمالي مع الاحتياطي** | **≈ 229 يوم عمل ≈ 11 شهرًا (مطوّر واحد)** | |

### مقارنة مع V1

| | V1 | V2 | الفارق |
|---|---|---|---|
| MVP بلا احتياطي | 140 | **199** | **+59 يومًا** |

**سبب الزيادة — بالتفصيل الصريح:**

| ⬆️ إضافات V2 | أيام | ⬇️ ما سقط من V2 | أيام |
|---|---|---|---|
| أولياء الأمور والوصاية | +12 | تقويم حجز حر للطالب | −8 |
| أجور المعلمين | +14 | محرك أهلية الحجز | −4 |
| تحقق التسليم | +6 | قائمة انتظار | −3 |
| الشهادات وتقدّم المستوى | +7 | بوابة تقديم المعلمين (D-28) | −6 |
| الترحيل | +9 | محرك امتحانات كامل (D-29) | −7 |
| المسار اليدوي للدفع | +6 | منطق مهلة الإلغاء (D-21) | −2 |
| الجداول المتكررة | +7 | | |
| **المجموع** | **+61** | **المجموع** | **−30** |

> ⚠️ **رسالة صريحة لصاحب المشروع:** قرارات S0 **صحيحة تجاريًا وضرورية**، لكنها **زادت حجم المشروع ~42%**. أي عرض سعر مبني على V1 **يجب إعادة تسعيره**. الاكتشاف الآن أرخص بكثير من اكتشافه في الشهر السادس.

**بنود خارج الرقم أعلاه:**

| البند | التقدير |
|---|---|
| تصميم UI/UX احترافي | +15–25 يومًا أو أتعاب مصمم |
| ⚖️ مراجعة قانونية + سياسات خصوصية القاصرين | أتعاب محامٍ — `LEGAL` |
| ⭐ **اختبار اختراق خارجي قبل الإطلاق** | **موصى به بشدة** — أموال + بيانات أطفال |
| صيانة سنوية | **15–20% من تكلفة التطوير سنويًا** |
| ⚠️ **تسوية حسابات الطلاب الحاليين** | ⚠️ **عمل تجاري لا برمجي** — يُقدَّر بعد §25.6 |
...
| ⭐ **Zoom** | 📈 **بعدد المضيفين** | **$42–71** (3–5 رخص Pro) | **$142–283** (10–20) |
| ⭐ **WhatsApp** | 📈 **بعدد الرسائل** | ⚠️ **UNKNOWN — انظر أدناه** | ⚠️ UNKNOWN |
| **بوابة الدفع** | 📈 بالإيراد | ⭐ **$0** إن بقي المسار يدويًا (D-11) | 0 أو 2.5–3.5% |
| **الإجمالي المعروف** | | **≈ $79–108 + واتساب** | **≈ $284–425 + واتساب** |
...
| ⭐ **تحديد المستوى** | جدولة عبر Zoom · تسجيل تقييم · **إسناد من معلم أو أدمن + تدقيق** (D-05/D-29) |
...
| **الاشتراكات** | 45 يومًا · **إنشاء إداري بلا دفع** · تمديد استثنائي مُدقَّق (D-13/D-16/D-18) |
...
| ⚠️ **الحصص الخاصة (Private)** | ⚠️ **غائبة عن S0 — `Q-09` قبل التأجيل النهائي** |
...
| تصحيح آلي للامتحان | التقييم بشري (D-29) |
...
| تصحيح آلي للـ Grammar | توفير وقت المعلمين | ❌ لا — بنك الأسئلة موجود |
...
| 12 | ⚠️ **+59 يوم عمل عن V1** | ⭐ إعادة تسعير العرض **قبل التوقيع لا بعده** |
...
| 17 | ⚖️ **بيانات قاصرين بلا مراجعة قانونية** | استشارة محامٍ **قبل الإطلاق لا بعده** |
| 18 | ⚠️ **حصة خاصة 1:1 مع طفل** | ⭐ سياسة سلوك + موافقة ولي أمر + ⚖️ مراجعة قانونية |
...
| **SOURCE B** | S0 §10 (D-16): الاشتراك صالح **45 يومًا** |
| **CONFLICT** | بحصتين أسبوعيًا (D-23)، 45 يومًا ≈ **12–13 حصة**. المستوى الواحد يمتد على **2–3 اشتراكات** |
...
| | |
|---|---|
| **SOURCE A** | S0 §12 (D-19): صلاحية التعويض **30 يومًا** |
| **SOURCE B** | S0 §10 (D-16): الاشتراك **45 يومًا** |
...
| **IMPACT** | 🔴 **يحدد سلوك دفتر الاستحقاقات** — هل يُنشأ قيد `Consumption` لطالب لم يحضر؟ محجوب لسلوك Phase 5 · **غير محجوب لشكل الجداول** |
| **CURRENT DECISION** | ⚠️ **غير محسوم** |
| **RECOMMENDED RESOLUTION** | التمييز بين ثلاث حالات: **(أ) عذر مقبول من الأدمن مسبقًا** ← لا استهلاك + تعويض · **(ب) غياب بلا إخطار (No-Show)** ← **يستهلك الاستحقاق** (وإلا انهار النموذج المالي: المعلم يُدفع له والطالب لا يُخصم منه) · **(ج) إلغاء إداري** ← نقل بلا حركة (D-20). ⚠️ **قرار تجاري** |
...
| **CONFLICT** | ⚠️ **غموض نطاق:** هل أُسقطت عمدًا أم سهوًا؟ |
| **IMPACT** | 🟠 ~5 أيام عمل + خطط أسعار + ⚖️ **اعتبار قانوني: طفل ومعلم منفردان** |
| **CURRENT DECISION** | ⚠️ **غير محسوم — `Q-09`** |
| **RECOMMENDED RESOLUTION** | **لا تفترض.** النموذج يدعمها بلا تكلفة (`session_type` + `capacity=1`). **اسأل صراحةً قبل تجميد النطاق** |
...
### `Q-02` — سياسة الغياب بمبادرة الطالب
...
### `Q-03` — حسم `CONF-01` (قاعدة الأهلية)
...
### `Q-04` — واقع بيانات الطلاب الحاليين
...
| `Q-09` | هل **الحصص الخاصة** ضمن MVP؟ (`CONF-09`) | ⚠️ **لا تفترض** · النموذج يدعمها بلا تكلفة · ⚖️ راجع بُعد «طفل ومعلم منفردان» |
| `Q-10` | ما **قائمة الدورات** المعتمدة في MVP بالضبط؟ | ابدأ بما يوجد له معلمون وأجور فعلًا |
| `Q-11` | امتحان تحديد المستوى: **مجاني أم مدفوع**؟ وهل تجوز الإعادة؟ بأي فاصل؟ | مجاني في MVP · إعادة بموافقة أدمن |
| `Q-12` | **الشهادة مجانية أم برسم**؟ | مجانية — أداة تسويق أقوى من كونها إيرادًا |
| `Q-13` | **العطل الرسمية · إجازات المعلمين · تجميد اشتراك الطالب** | جدول `holidays` + `teacher_time_off` · **التجميد يؤجَّل** (يعقّد صلاحية الـ45 يومًا) |
...
| `Q-17` | هل يرى ولي الأمر **كل شيء** عن ابنه 17 سنة؟ ولمن تذهب إشعارات القاصر؟ | (أ) رؤية كاملة حتى 18 للبساطة القانونية · ⚖️ **مراجعة محامٍ** |
| `Q-18` | ⚖️ **أدمن في بريطانيا يصل لبيانات أطفال في الأردن/فلسطين** — نقل بيانات دولي | ⚖️ **`LEGAL` — لم يُطرح في S0 ويجب أن يُطرح** |
| `Q-19` | ⚖️ **مدة الاحتفاظ بالبيانات** وسياسة الحذف مع الالتزام المحاسبي | إخفاء الهوية مع حفظ السجل المالي (§32.4) · ⚖️ إقرار محامٍ |
...
| `Q-22` | أجر المعلم **بالساعة الكاملة أم بالدقائق**؟ (§18.4) | الساعة الكاملة للحصص القياسية |
| `Q-23` | أجر عن **حصة أُلغيت متأخرًا**؟ (§18.5) | قرار أدمن مُدقَّق في MVP + نص في عقد المعلم |
| `Q-24` | **عتبات درجات** A1–C2 (§10.4) | هجين: النظام يقترح والمعلم يعتمد |
...
| `Q-29` | حسم `CONF-04`: التعويض 30 يومًا مقابل الاشتراك 45 يومًا | التعويض مستقل عن الاشتراك |
...
| 2 | 🔴 ⭐ **اتصال بـ PayTabs الأردن و MEPS فلسطين: «هل تقبلون تاجرًا فردًا بحساب شخصي؟»** | المالك | ⏳ أيام |
| 3 | ⏳ ⭐ **بدء توثيق Meta Business + طلب WhatsApp Business API** | المالك | ⏳ **أسابيع** |
| 4 | ⭐ **طلب عيّنة 10 طلاب حاليين بكل حقولهم** | المالك | يوم |
| 5 | 🔴 **طلب وثيقة SRS كاملة — الصفحتان 1 و3 ما زالتا مفقودتين** | المالك | يوم |
...
| 10 | ⚖️ **حجز موعد مع محامٍ لخصوصية القاصرين** | المالك | ⏳ أسابيع |

> ⭐ **البنود 2 و3 و6 و10 لها مدد انتظار خارجية.** ابدأها **اليوم** — تأخيرها يؤخر الإطلاق مهما تقدّم الكود.
...
| **3. الكتالوج والمستوى** | 2.5 أسبوع | بلدان · دورات · مستويات · فئات · أسعار · امتحان · إسناد مستوى | ✅ **T-12 (تلاعب البلد) يمر** |
| **4. الجدولة** ⚠️ | 4 أسابيع | جداول متكررة · مولّد · إتاحة · `EXCLUDE` · تسجيل · حضور | ✅ **T-01 · T-02 · T-03 تمر** |
| **5. الدفتر والاشتراكات** ⚠️⚠️ | 4 أسابيع | دفتر الاستحقاقات · اشتراكات · 45 يومًا · **الاشتراك الإداري** | ✅ **T-04 · T-05 · T-06 تمر** ⚠️ **يتطلب حسم `Q-02` و`Q-03`** |
...
| **11. الإدارة والتقارير** | 2.5 أسبوع | لوحة · فلاتر · تدقيق · إعلانات | ✅ **T-13 يمر** |
| **12. التشديد والإطلاق** | 2.5 أسبوع | أمن · أداء · نسخ · مراقبة · **اختبار استعادة** | ✅ **T-16 يمر** · ⭐ اختبار اختراق |

**الإجمالي ≈ 35 أسبوعًا ≈ 8 أشهر عمل صافٍ · مع الاحتياطي ≈ 11 شهرًا.**
...
**مطوّر ASP.NET Core متمكّن ينفّذه في ≈ 8 أشهر عمل صافٍ، و≈ 11 شهرًا بالاحتياطي الواقعي.**
...
| `CONF-03` | يظهر فقط عند حساب 30 ساعة ÷ 45 يومًا |
| `CONF-04` | يظهر فقط عند تقاطع 30 يومًا مع 45 يومًا |
...
| **الـ MVP** | §39.1 — دورة كاملة: تسجيل ← امتحان ← مستوى ← اشتراك ← دفع ← جدول ← حصة ← تسليم ← أجر ← شهادة |
| **تكلفة التطوير** | **≈ 199 يوم عمل** (≈ 229 بالاحتياطي) ≈ **11 شهرًا لمطوّر واحد** |
| **التكلفة الشهرية** | **≈ $79–108 + فاتورة واتساب** لـ MVP (≤500 طالب) |
...
4. ⚖️ ⭐ **بيانات أطفال من عمر 5 سنوات، وحصص خاصة 1:1، وأدمن في بلد ثالث — بلا مراجعة قانونية.** هذا ليس بندًا مؤجَّلًا: **حجز موعد المحامي يبدأ في الأسبوع صفر.**
...
| 5 | الاشتراك 45 يومًا متسق | ✅ | §19.2 · §19.3 · `CONF-03` · `CONF-04` |
