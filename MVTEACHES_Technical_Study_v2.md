...
|---|---|
| **الإصدار** | **v3.0** |
| **تاريخ الإصدار** | 2026-08-17 · مُحدَّثة 2026-08-19 · ⭐ **مُحدَّثة 2026-08-24** |
| **يحل محل** | `MVTEACHES_Technical_Study_v1.md` (v1.0) |
| **الحالة** | Pre-Development — Implementation Blueprint |
| **الأساس الجديد** | «OFFICIAL BUSINESS DECISIONS» (2026-08-17) + 10 قرارات (2026-08-19) — D-38…D-47 + ⭐ **19 قرارًا من الجولة الثالثة (2026-08-24)** — D-48…D-66 |
...
## 8.2 ✅ الغياب بمبادرة الطالب — محسوم (`D-42`)

الطالب الذي **لا يحضر دون عذر** (No-Show)، أو **يعتذر قبل الحصة**، له سياسة محسومة منذ 2026-08-19:

| الحالة | استحقاق الطالب | التعويض |
|---|---|---|
| عذر مقبول من الأدمن **قبل** الحصة | ❌ لا يُستهلك | ✅ حصة بديلة تُجدوَل |
...
|---|---|---|---|---|
| 1 | ✅ | ✅ | ✅ يُدفع | ✅ يُستهلك |
| 2 | ✅ | ❌ (تخلّف بلا عذر) | ✅ **يُدفع** | ✅ **يُستهلك ويضيع** (`D-42`) |
| 2ب | ✅ | ⚠️ (تخلّف بعذر مقبول) | ✅ **يُدفع** | ❌ لا يُستهلك — **حصة بديلة يجدولها الأدمن** (`D-42`/`D-55`) |
| 3 | ❌ (المعلم غاب) | — | ❌ **لا يُدفع** | ❌ **لا يُستهلك** |
| 4 | ❌ (أُلغيت إداريًا) | — | ⚠️ `Q-07` | ❌ لا يُستهلك — يُنقل (D-20) |

> **الصف 2 هو الحالة التي تكشف صحة التصميم.** المعلم حضر وانتظر وأعطى الحصة لثلاثة طلاب من أربعة. **يستحق أجره كاملًا** — لأن أجره على الساعة المُسلَّمة لا على عدد الحضور (D-26). ⭐ **واستحقاق الطالب المتخلف محسوم بـ`D-42`**: بلا عذر ← يُستهلك ويضيع؛ بعذر مقبول ← لا يُستهلك ويُعوَّض. **الحاسم في التصنيف: هل قُبل العذر من الأدمن قبل الحصة أم لا — لا أي معيار آخر.**
>
...
| `Absent` | لم يحضر بلا عذر | ✅ يُستهلك ويضيع — لا تعويض (`D-42`) |
| `Excused` | عذر مقبول من الأدمن (قبل الحصة) | ✅ لا يُستهلك — حصة بديلة تُجدوَل (`D-42`) |
...
> ⚠️ **الرقم الإجمالي (≈192 يومًا) لم يُحدَّث بعد.** إعادة التقدير تتم مرة واحدة بعد استقرار هذه الجولة، لا على دفعات.

### ما يزال مفتوحًا بعد v3.0

| # | البند |
|---|---|
| ~~1~~ | ✅ ~~سلّم درجات الشهادة ودرجة النجاح~~ — **حُسم 2026-08-25**: المعلم/الأدمن يحدّدها حرًّا بلا رقم مفروض من النظام (`D-52`) |
...
| ~~4~~ | ✅ ~~صفحتا SRS رقم 1 و3~~ — **حُسم 2026-08-25**: Branch = Country بتأكيد المالك، الصفحتان أُسقطتا نهائيًا (§0) |
