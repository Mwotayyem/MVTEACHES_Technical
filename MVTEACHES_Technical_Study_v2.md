...
**قرار تصميمي:** المستويات **بيانات في جدول** لها `sort_order`، **لا Enum في الكود**.
...
> ⛔ **تصحيح (2026-08-26، وُجد أثناء بدء التنفيذ):** كان هذا الجدول يحمل عمود `required_minutes` لعتبة الشهادة لكل مستوى. **هذا يتعارض فعليًا مع `D-65`/§19.5**، الذي حسم عتبة الشهادة كـ**إعداد عام واحد** (`settings.CertificateRequiredHours`) يُقرأ لحظيًا لكل المستويات معًا — لا عمود لكل مستوى، ولا نسخة لكل طالب. `D-65` قرار لاحق وأكثر تحديدًا، وهو الذي يسود. العمود **أُسقط من هذا الجدول**؛ عتبة الشهادة تُقرأ حصرًا من §19.5 لكل الأنظمة (طالب × مستوى) دون استثناء.
...
> ### ⛔ ملغى ومستبدَل بتاريخ 2026-08-29 — اقرأ D-91 قبل الاعتماد على ما يلي
>
> توضيح المالك (2026-08-29) ألغى فرضية **حساب Zoom مركزي واحد** وفرضية
> **Server-to-Server OAuth** بالكامل. القاعدة السارية: كل معلم يربط **حسابه
> الشخصي** (Zoom أو Google Meet) عبر OAuth مفوَّض من المستخدم، ولا يوجد
> `AccountId` مركزي، ولا تتحمل المؤسسة أي اشتراك. **حساب Google مجاني يكفي** —
> لا يُطلب اشتراك مدفوع من أي نوع. المعلم الذي لا يملك أيًا منهما يُعلَّم
> "غير جاهز للحصص الأونلاين" ولا يمكن إسناد أي حصة له.
> التفاصيل الكاملة في `MVTEACHES_Owner_Answers_R3.md` (D-91) وفي
> `docs/deployment/STATUS.md`.
>
> ما يلي محفوظ كسجلّ تاريخي فقط. ملاحظة مهمة: قاعدة `start_url` أدناه
> **لا تزال سارية بالكامل** ومطبَّقة فعليًا في التنفيذ الحالي.

> ### ~~✅ القرار: اجتماع مُولَّد لكل حصة عبر Server-to-Server OAuth~~ (ملغى)
>
> | البند | القرار |
> |---|---|
...
| ~~⭐ حضور غير مسجَّل بعد 24 ساعة~~ | ⛔ **سقط بـ`D-83`** — الحضور ذاتي الخدمة (Join)، لا تسجيل يدوي يُنسى | — | — | — |
