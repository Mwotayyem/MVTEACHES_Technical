...
## ⭐⭐⭐⭐ Owner decision (2026-08-30) — placement test, level-gated packages, entitlement transfer, teacher-level/slot UI, notifications, financial dashboard

**This section is the latest pass and supersedes two specific rows in the
section immediately below it: the `D-99` row's "admin UI still missing" note
(the admin teacher-level screen and the teacher-facing slot-publishing screen
are both built now), and reverses `D-48` in `MVTEACHES_Owner_Answers_R3.md`
(kept there, marked superseded, not deleted) after the reversal was raised
and explicitly re-confirmed by the owner before any code was written.
Decisions `D-101`…`D-109` in `MVTEACHES_Owner_Answers_R3.md` are the
authoritative wording; this table is what actually exists in code.

| Rule | Status | What exists now |
|---|---|---|
| **1 — Free placement test gates purchase** (`D-101`/`D-102`) | ✅ Implemented | A student/guardian must complete `IPlacementAttemptService`'s free first attempt before any package purchase is possible — enforced server-side in `ISubscriptionService.PurchaseFromPlanAsync` (`StudentHasNoAssignedLevel`), not merely by hiding the purchase page's UI. `/PlacementTest` (shared Student/Guardian page) and `/PurchasePackage` (shows the CTA to take the test instead of a package list when no level is assigned yet) are both built and tested end to end, including a regression proving a tampered direct POST past the CTA is still refused with no `Subscription` row ever created. |
| **2 — Placement-test authoring, no invented content** (`D-101`) | ✅ Implemented | `IPlacementTestAdminService` (draft/published lifecycle, questions/choices/points, score ranges, publish validation collecting every error at once, activate-one-version) plus `/Admin/PlacementTests`. Correctness is never sent to the browser before submission (`PlacementQuestionOption` has no field capable of holding it — verified by reflection in a test). Scoring and level assignment happen entirely server-side in `SubmitAttemptAsync`. Every completed attempt (version, answers, score, level, time) is preserved append-only — a later edit to the question bank never rewrites history. |
| **3 — Retakes and admin override, audited** (`D-101`) | ✅ Implemented | First attempt free; `AlreadyCompletedNoRetakeApproved` blocks a second attempt until `PlacementRetakeRequest` is admin-approved; an approved retake creates a new `PlacementAttempt`, never edits the old one. `OverrideLevelAsync` requires a reason and writes an `AuditLogEntry`. Every guardian-facing action re-verifies "self or active guardian" the same way `JoinAttendanceService` does — a stranger cannot see or act on another family's attempt (regression-tested). |
| **4 — Packages tied to one level + one session type** (`D-95`/`D-103`) | ✅ Implemented | `PricingPlan.LevelId`/`SessionType` are shown on every card and enforced end to end: `PurchaseFromPlanAsync` derives the level from the plan (never a caller-supplied `levelId` — the trust gap that used to exist), and `SubscriptionSessionType`/`EntitlementLedgerEntry.SessionType` (added this pass) mean a Group entitlement can never fund a Private session or vice versa, in booking, Join, or the manual "تسجيل باقة ودفع يدوي" path alike. Online payment stays externally blocked, exactly as `D-94` already stated — nothing here simulates or falsely confirms one. |
| **5 — No silent level-change conversion; admin-only transfer** (`D-104`) | ✅ Implemented | Nothing needed to change to make an old-level entitlement structurally unusable at a new level — `EntitlementLedgerEntry.LevelId` already scoped every balance/consumption query. New `IEntitlementTransferService`: `GetLockedBalancesAsync` (surfaces what's stuck), `TransferMinutesAsync` (moves an exact amount into an **existing** current-level subscription only — never fabricates one — behind a mandatory reason, a row-lock-then-recompute concurrency guard, and two net-zero, append-only `AdminAdjustment` ledger entries plus an `AuditLogEntry`). |
| **6 — Admin teacher-level assignment UI** (`D-99`/`D-105`) | ✅ Implemented (closes the `D-99` gap) | `/Admin/Teachers` extended: grant/revoke a level per teacher (duplicate grants refused server-side by `ux_teacher_level`, not just hidden), each currently-authorized level shown as a one-click revoke, alongside the pre-existing Zoom/Google readiness indicator. `RevokeAsync` gained an acting-user id and both directions are now audit-logged (they weren't before this pass). |
| **7 — Teacher slot publishing** (`D-99`/`D-106`) | ✅ Implemented (closes the `D-99` gap) | New `ITeacherSlotPublishingService` — distinct from the admin-only `RecurringScheduleService` weekly roster and from `TeacherAvailabilityRule`/`TeacherTimeOff` (which describe when a teacher is *not* available, not a bookable slot). Re-checks teacher-self ownership, video readiness, and level authorization server-side; capacity is always derived from `SessionType`, never accepted from the request; overlap is the existing `no_teacher_overlap` EXCLUDE constraint. `/Teacher/PublishSlots` is the UI, with its Level dropdown pre-filtered to the teacher's own grants. |
| **8 — Booking-flow audit** (`D-107`) | ✅ Audited, no defect found | Reviewed the complete production flow — child selection, level/type/package filtering, no double payment, capacity/minutes/concurrency checks, safe rejection. Confirmed booking itself never debits minutes (only a real Join or a finalized no-show does, per `D-83`/`D-95`); no change was needed here beyond the `SessionType` scoping already covered under rule 4. |
| **9 — Notification wiring + financial dashboard** (`D-108`/`D-109`) | ⚠️ Mostly implemented; one item deliberately deferred | Four previously-unfired events now fire at the point their underlying write succeeds (same transaction/batch, so a rollback never leaves an orphaned notification): `SubscriptionConfirmed` (purchase), the new `BookingConfirmed`, `SessionCancelledOrMoved` for the **general** cancellation path (not just teacher reassignment), and the new `CompensationRejected`. A new idempotent `SessionReminderJob` (1-minute recurring Hangfire job) fires `ZoomLink5Min` exactly once per (session, recipient) via a new `NotificationOutboxItem.SessionId` + supporting index. Financial dashboard gained scheduled-teaching-minutes, admin-entered `OperatingExpense`s (the "Payroll" category is rejected outright, both in the domain constructor and the service, so payroll can never be double-counted), net profit per currency, and month-over-month comparison. **Deferred:** the periodic "upcoming summary" digest — `SessionReminderStudent` exists as an event type but nothing schedules it yet; left for a follow-up rather than guessing at a cadence/window the owner didn't specify. |

### Not done in this pass — stated plainly

- **The periodic "upcoming summary" notification job.** `SessionReminderStudent` is a real event type; no recurring job produces it yet. The 5-minute-before reminder (a concretely specified cadence) was built; this one wasn't specified precisely enough to build without guessing, so it wasn't guessed.
- **No payment provider has been selected** — unchanged from the pass below. Every purchase this pass creates stays a Draft subscription; the page says so plainly rather than claiming a payment succeeded.
- **No live WhatsApp delivery** — unchanged. `notification_outbox` is the durable record of intent; sending real messages needs Meta credentials/templates that don't exist in this environment.
- **No live Zoom/Google verification for the new teacher-slot-publishing path** — it reuses the same `IMeetingProvisioningService`/OAuth boundary every other scheduling path already goes through, and that boundary's own live-credential gap is unchanged from the section below.

**Verification for this pass:** the complete real-PostgreSQL suite was run repeatedly across every batch in this pass (never once, and never skipped after a change), landing at **349 passed / 0 failed**, and every new migration was verified the same way every migration in this project always is — the test database is dropped, recreated, and migrated from empty on every single run, so a broken migration chain would show up as an immediate, total test failure, not a claim taken on faith.
